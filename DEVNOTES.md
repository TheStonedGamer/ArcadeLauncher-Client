...
**`WinHttpCrackUrl` cannot parse `ws://` / `wss://` — feed it `http(s)://`.** (This was the real cause of the long-standing "Reconnecting…" gateway bug.)
`WsUrl()` builds a `wss://host/ws/social?token=…` URL, but `WinHttpCrackUrl` does not understand the `ws`/`wss` schemes: it returns `INTERNET_SCHEME_UNKNOWN` with `nPort == 0`. The old code then set `secure = (nScheme == INTERNET_SCHEME_HTTPS)` → `false` and called `WinHttpConnect(host, 0)`, which fails before any socket opens — so the upgrade request never left the machine (telltale: `/api/social/*` REST returns 200 but there are **zero** `/ws/social` lines in the nginx access log). The WinHTTP WebSocket upgrade actually rides on a normal `http(s)` request with `WINHTTP_OPTION_UPGRADE_TO_WEB_SOCKET`, so `WebSocketClient::Run` now maps `wss://`→`https://` and `ws://`→`http://` *before* `WinHttpCrackUrl`. Verify a fix by grepping nginx for a `101` with UA `ArcadeLauncher/SocialWS`.

**The social base URL must carry an explicit scheme.**  
`config.serverBaseUrl` is often stored schemeless (`arcade.orlandoaio.net`). `ServerClient` normalizes that to `https://` internally, but only on *its own* copy — so `SocialManager::Start` used to receive a bare host. Without a scheme the social **REST** helpers' `WinHttpCrackUrl` rejected the URL outright (no friends list), and `WsUrl()` couldn't produce a `wss://` (it keys off the `https://` prefix). `SocialManager::Start` now runs its own `NormalizeOrigin` (https:// for public hosts, http:// for `10.`/`192.168.`/`127.`/`localhost`). Any caller passing a host into the social subsystem must not assume the scheme is added elsewhere. (Necessary, but the gateway still needed the `WinHttpCrackUrl` fix above to actually connect.)
